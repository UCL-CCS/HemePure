...
namespace hemelb
{
	...
	namespace colloids
	{
		...
		const MPI_Datatype Particle::CreateMpiDatatypeWithPosition() const
		{
			...
			// we have chosen to make each block of fields contain a single field
			// so, the number of field blocks is the same as the number of fields
			int numberOfFieldBlocks = 9;

			// and the length of every field block is one
			int lengthOfEachFieldBlock[] = { 1, 1, 1, 1, 1, 1, 1, 1, 1};
			...
			MPI_Get_address(& (temp.voxelSize), & (displacementOfEachFieldBlock[3]));
			MPI_Get_address(& (temp.timeStepLength), & (displacementOfEachFieldBlock[4]));
			MPI_Get_address(& (temp.lastCheckpointTimestep), & (displacementOfEachFieldBlock[5]));
			MPI_Get_address(& (temp.creationTimestep), & (displacementOfEachFieldBlock[6]));
			MPI_Get_address(& (temp.markedForDeletionTimestep), & (displacementOfEachFieldBlock[7]));
			MPI_Get_address(& (temp.globalPosition), & (displacementOfEachFieldBlock[8]));

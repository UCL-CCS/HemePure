...
namespace hemelb
{
	namespace colloids
	{
			...
			ioComms(ioComms_), simulationState(simulationState), timers(timers)
		{
			...
			configuration::GetDimensionalValue(
					particlesElem.GetChildOrThrow("emissionItrvl"), "dimensionless", emissionItrvl);

			// is particle emission enabled?
			emissionFlag = (emissionItrvl > 0);

			particleSet = new ParticleSet(latDatLBM, particlesElem, propertyCache, simConfig, neighbourProcessors, ioComms, outputPath);
		}
				...
				const Neighbourhood& neighbourhood)
		{
					...
					blockTraverser.TraverseOne())
			{
				...
				// if block has no sites
				if (gmyResult.Blocks.find(blockId) == gmyResult.Blocks.end())
				{
					log::Logger::Log<log::Debug, log::OnePerCore>(
							"DEBUG ColloidController: block with id %i and coords (%i,%i,%i) is solid.",
							blockId,
						...
						siteTraverser.TraverseOne())
				{
					...
					if (gmyResult.Blocks.at(blockId).Sites[siteId].targetProcessor != this->ioComms.Rank())
					{
						log::Logger::Log<log::Trace, log::OnePerCore>(
								...
								gmyResult.Blocks.at(blockId).Sites[siteId].targetProcessor);
						continue;
					}
				...
				proc_t* ownerRankForSite)
		{
		  ...
		  //if (gmyResult.Blocks.at(*blockIdForSite).Sites.empty())
			if (gmyResult.Blocks.find(*blockIdForSite) == gmyResult.Blocks.end())
				return false;
			...
			*ownerRankForSite = gmyResult.Blocks.at(*blockIdForSite).Sites[*localSiteIdForSite].targetProcessor;
		...
		void ColloidController::RequestComms()
		{
			...
			particleSet->CommunicateParticlePositions(currentTimestep);
			timers[reporting::Timers::colloidCommunicatePositions].Stop();
#ifndef HEMELB_TRACER_PARTICLES
			// first use of UpdatePosition() (in constructor) does not find ghost particles
			// ranks will not have necessary particles for interactions and feedback
			// therefore, do not perform interactions or calculate feedback until currentTimestep > 1
			if (currentTimestep > 1) {
				log::Logger::Log<log::Info, log::Singleton>(
						"body forces will be applied!");
		...
		void ColloidController::EndIteration()
		{
			...
			if (currentTimestep > 1) {
				...
				particleSet->InterpolateFluidVelocity(currentTimestep);
				timers[reporting::Timers::colloidUpdateCalculations].Stop();
				timers[reporting::Timers::colloidCommunicateVelocities].Start();
				particleSet->CommunicateFluidVelocities(currentTimestep);
				timers[reporting::Timers::colloidCommunicateVelocities].Stop();
#ifndef HEMELB_TRACER_PARTICLES
				particleSet->UpdateVelocities();
#endif
				particleSet->ApplyBoundaryConditions(currentTimestep);
			} else particleSet->ResetVelocities();
			if (currentTimestep % 250 == 0)
				particleSet->OutputInformation(currentTimestep);
			if (currentTimestep % emissionItrvl == 0 && emissionFlag)
				particleSet->EmitParticles(currentTimestep);
			particleSet->UpdatePositions(currentTimestep);
		}
	}
}
